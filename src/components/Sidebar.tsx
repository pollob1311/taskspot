...
export default function Sidebar({ isCollapsed, setIsCollapsed }: SidebarProps) {
    ...
    // টাইমওয়ালের বেস ইউআরএল এবং ইউজার আইডি
    ...
    const menuItems = [
        ...
        // --- এই অংশটি পরিবর্তন করা হয়েছে (Surveys) ---
        {
            label: 'Surveys',
            icon: Zap,
            href: `${timeWallBase}&uid=${userId}&tab=surveys`,
            isExternal: true
        },
    ...
    return (
        <aside className={`fixed left-0 top-0 h-screen bg-[#CAD5E2] dark:bg-[#364153] border-r border-black/5 dark:border-white/10 flex flex-col z-50 backdrop-blur-xl transition-all duration-300 ${isCollapsed ? 'w-7' : 'w-[70px] md:w-48 xl:w-64'}`}>
            ...
            <nav className={`flex-grow pb-4 space-y-1 overflow-y-auto no-scrollbar ${isCollapsed ? 'px-0' : 'px-4'}`}>
                {menuItems.map((item) => {
                    const commonClasses = `flex flex-col md:flex-row items-center ${isCollapsed ? 'justify-center' : 'justify-center md:justify-start'} gap-1 md:gap-3 ${isCollapsed ? 'px-0' : 'px-1 md:px-4'} py-2 md:py-2.5 rounded-xl text-sm font-bold group relative ${item.active
            ...
            <div className="p-4 border-t border-slate-800 space-y-4">
                <button
                    onClick={toggleTheme}
                    className={`w-full flex flex-col md:flex-row items-center ${isCollapsed ? 'justify-center' : 'justify-center md:justify-start'} gap-1 md:gap-3 ${isCollapsed ? 'px-0' : 'px-1 md:px-4'} py-3 rounded-xl text-sm font-bold text-[var(--text)]/60 hover:text-[var(--text)] hover:bg-black/5 dark:hover:bg-white/5 transition-all group relative`}
                >
                    {theme === 'dark' ? (
                        <>
                ...
                <div className={`flex flex-col md:flex-row items-center ${isCollapsed ? 'justify-center' : 'justify-center md:justify-start'} gap-1 md:gap-3 ${isCollapsed ? 'px-0' : 'px-1 md:px-4'} py-2 bg-black/5 dark:bg-white/5 rounded-2xl group relative`}>
